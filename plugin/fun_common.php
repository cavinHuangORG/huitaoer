...
{
	list($type,$className)=explode('_', $name);
	if($type=='mod')
	{
		$path=PATH_MODEL.'/'.$className.'.mod.php';
	}
	elseif($type=='lib')
	{
		$path=PATH_LIBRARY.'/lib/'.$name.'.php';
	}
	require $path;
}
...
{
	/*判断是否是vip会员，extend下面'album普通会员','gift普通会员','seller卖家组',*/
...
function brandgather(){
	...
	if($type=='brand'){
		...
		$pic_url=$data['pic'];
		lib_database::wquery('replace into '.tname('brand').'(`bid`,`brand`,`preferential`,`url`,`nick`,`remark`,`logo`,`pic`,`start`,`end`,`addtime`,`sort`,`sbid`) 
			VALUES(\''.intval($brand['bid']).'\',\''.$data['brand'].'\',\''.$data['preferential'].'\',\''.$data['url'].'\',\''.$data['nick'].'\',\''.$data['remark'].'\',\''.$logo_url.'\',\''.$pic_url.'\',\''.$data['start'].'\',\''.$data['end'].'\',\''.$data['addtime'].'\',\''.intval($data['sort']).'\',\''.$data['bid'].'\')');
...
{
	if(empty(lib_request::$files['imgFile'][0]))
	{
		$picname = lib_request::$files['imgFile']['name'];
		$picsize = lib_request::$files['imgFile']['size'];
		if ($picname != "") {
			if ($picsize > 1024000) {
				echo json_encode(array('error'=>'-1','message'=>'图片大小不能超过1M'));
				exit();
			}
			$type = get_extension($picname);
			if ($type != "gif" && $type != "jpg" && $type != "jpeg" && $type != "png") {
				echo json_encode(array('error'=>'-1','message'=>'图片格式不对！'));
				exit();
			}
			$rand = rand(100, 999);
			$pics = date("YmdHis") . $rand . '.'.$type;
			...
			$pic_path = PATH_ROOT.$pic_url;
			move_uploaded_file(lib_request::$files['imgFile']['tmp_name'], $pic_path);
		}
		echo json_encode(array('error'=>0,'url'=>get_img($pic_url)));
	}
	else
	{
...
function get_taoke($iid,$rf=''){
	...
	$pgid=md5($iid);
	if(empty($rf)){
		$rf=urlencode(u('index','jump',array('iid'=>$iid),true));
		if(!preg_match('/^http/',$rf)){
			$rf=urlencode($_webset['site_url'].'/').$rf;
		}
	}
	$url='http://g.click.taobao.com/load?rf='.$rf.'&dr=&pid='.$pid.'&pgid='.$pgid.'&ak=&ttype=1&iframe=false&st=1443168737341%2C1443168737344&lan=0%2C1&ciid=17309968572&csid=&curl=&ckeywords=&cbh=612&cbw=1920&re=1920x1080&cah=1040&caw=1920&ccd=24&ctz=8&chl=2&cja=1&cpl=5&cmm=7&cf=18.0&cb=jsonp_callback_004967557514815568';
	...
	if($b[1]!=''){
		...
		$b[1]=implode('',$b[0]);
		$url='http://g.click.taobao.com/display?rd=2&pid='.$pid.'&pgid='.$pgid.'&rf='.$rf.'&et='.$b[1].'&ttype=1&v=1.2&cm=&ck=&cw=0&ct=itemid%3D'.$iid.'&wt=0&tl=230x312&st=2&cb=jsonp_callback_08553375299088657';
...
function get_img($url,$w=''){
	...
	preg_match("/^(https?:\/\/).*?/",$url,$server);
	if(!empty($server[1])){
		return $url.$w;
	}else{
		$num=substr_count(dirname($_SERVER['SCRIPT_FILENAME']).'/','/')-substr_count(str_replace('\\', '/',PATH_ROOT), '/');//计算网站根目录与当前目录相差级数
		$sub='';
		for ($i=0; $i <$num ; $i++) { 
			$sub.='../';
		}
		if (file_exists($sub.$url.$w)) {
			return SERVER.$sub.$url.$w;
		}else{
			return $sub.$url;
		}
	}
 ...
 * 验证用户是否对一个商品已申请试用
 * @parem $goods_id 商品id
 * @return 返回是否申请了试用 true申请过试用false没申请过
 */
function is_trygoods($goods_id)
{
	global $user;
	//如果用户已登录判断用户是否已申请试用
	if(!empty($user['uid'])){
		$applylog=lib_database::select('applylog','count(*) as ct','id=\''.$goods_id.'\' and uid='.$user['uid']);
		return $applylog[0]['ct'];//试用记录大于0则已申请试用
	}
	else
	{
		return false;//未登录也当做没有申请试用
	}
}
//采集规则列表
function autogathertasklist() {
