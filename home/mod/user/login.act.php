...
if(empty($gourl))
{
	$gourl =base64_encode(u('user','base'));//将字符串以 MIME BASE64 编码
}
if(is_bool(base64_decode($gourl)))
{
	$gourl=urlencode(base64_encode($gourl));
}
$login=request('login');
if(!empty($login)){
	//print_r($login);exit();
	...
	{	
		...
		if(!empty($login['verify']))
		{
			if(strtolower($login['verify'])!=strtolower($_SESSION['ckstr'])){
				message(-1,'登陆提示','验证码错误！');
			}
		}
		//try
		//{
