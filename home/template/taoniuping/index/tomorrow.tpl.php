        ...
        <?php foreach ($goods['data'] as $key => $value) { ?>
            <li>
                <div class="deal dealbig">
                    ...
                    <?php } ?>
                    <h3 class="stnmclass">
                        <?php if (empty($_webset['base_tomorrow_jump'])) { ?>
                            <a href="javascript:;">
                            <?php } else { ?>
                    ...
                    <div class="beauty_pro_info <?php if ($value['start'] > $_timestamp) { ?>unstart<?php } elseif ($value['end'] < $_timestamp) { ?>end<?php } ?>">
                        <em class="ptitle">
                            <?php if (empty($_webset['base_tomorrow_jump'])) { ?>
                                <a href="javascript:;">
                                <?php } else { ?>
                                    <a <?= gogood($value['num_iid']); ?> num_iid="<?= $value['num_iid']; ?>" title="<?= $value['title']; ?>">
                                    <?php } ?>
                                    <?php if ($value['ispost'] == 1) { ?>【包邮】<?php } ?><?= $value['title']; ?>
                                </a>
                        </em>
                        <span class="price_list_sale"> ￥ <em><?= trim_last0($value['promotion_price']); ?></em><del class="f14 gray">￥<?= trim_last0($value['price']); ?></del></span>
                        <?php if (empty($_webset['base_tomorrow_jump'])) { ?>
                            <a href="javascript:;" class="beauty_link_b">
                            <?php } else { ?>
