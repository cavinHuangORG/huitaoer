...
//通过初审
function trial(id,type){
	ajaxOperating('?mod=ajax&ac=operat&op=trial',{'id':id},'GET','jsonp','trial_callback');
}

function trial_callback(json)
{
	show_box("提示",json.msg);
...
*/
//设置主题图片
function setworthpic(json){
	var filename=$("#fileupload").attr("name");
	$("input[name='worth[pic]']").val(json[filename].pic);
	$(".box-content .img img").attr("src",'../'+json[filename].pic);
}
//设置试用图片
