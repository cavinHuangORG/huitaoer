...
<ul class="nav">
	<li <?=$active['list'];?>><a href="?mod=<?=MODNAME;?>&ac=<?=ACTNAME;?>&op=list">待审核兑换</a></li>
	<li <?=$active['refuse'];?>><a href="?mod=<?=MODNAME;?>&ac=<?=ACTNAME;?>&op=refuse">拒绝的兑换</a></li>           
</ul>
    ...
    <div class="table">
        ...
        <tr id="data_<?=$value['id'];?>">
        	...
        	<td class="text-center">
            	...
            	[<a href="?mod=exchange&ac=index&op=excAdd&id=<?=$value['id'];?>">修改</a>]&nbsp;
            	<?php }else{ ?>
            	<?php if($value['status']==0){ ?>
            	[<a href="javascript:;" onclick="refuse('<?=$value['id'];?>','exchange');">拒绝</a>]&nbsp;
            	<?php } ?>
         		[<a href="javascript:;" onclick="audit('<?=$value['id'];?>','exchange')">通过排期</a>]&nbsp;
            	<?php } ?>
            </td>
        </tr>
        <?php if($value['status']==-1){ ?>
