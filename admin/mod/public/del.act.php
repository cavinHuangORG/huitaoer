...
switch ($op){
	...
	case 'worth'://删除主题
		//删除图片
		lib_database::rquery('select pic from '.tname('worth').' where wid in ('.$idstr.')');
		while ($value=lib_database::fetch_one()){
			if(!empty($value['pic']) && check_img($value['pic'])){
				@unlink(PATH_ROOT.$value['pic']);
				@unlink(PATH_ROOT.$value['pic'].'_290x190.jpg');
		...
		lib_database::query('delete  from '.tname('worth').' where wid in('.$idstr.')');
		//
		$gobid=request('gobid',0);
		$gourl='?mod='.$gomod.'&ac='.$goac.'&op='.$goop.'&bid='.$gobid;
		break;
	case 'type'://删除分类
		...
		lib_database::delete('type_gather','cid in ('.$idstr.')');
		del_cache('help','cat');
		del_cache('goods','cat');
