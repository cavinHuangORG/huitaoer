...
function upsort($id, $sort, $type) {
    if ($type == 'goods') {
       ...
       lib_database::wquery('update ' . tname('worth') . ' set `sort`=' . $sort . ' where wid=' . $id);
    }elseif ($type == 'cat') {
...
function trial($id) {
    $data = array();
    $data['status'] = 2;
    return lib_database::update('goods', $data, 'id=' . $id);
}
...
function refuse($data, $type) {
    ...
    if (V_MODE == 'vip') {
        ...
        //获取商品信息
        $goods_order = lib_database::get_one('select a.pay_serialno,a.pay_money,a.id,a.title,a.status,b.uid,b.user_name,b.email,b.mobile from ' . tname('goods') . ' as  a left join ' . tname('users') . ' as b ON a.uid=b.uid WHERE a.id=' . $data['id']);
        $goods_order['user_name']=empty($goods_order['user_name'])?(empty($goods_order['email'])?$goods_order['mobile']:$goods_order['email']):$goods_order['user_name'];
        $goods_order['url']=  u('seller','manage');
        //如果是付费用户
        if (!empty($goods_order['pay_serialno']) && !empty($goods_order['uid']) && floatval($goods_order['pay_money']) > 0) {
            //将金额返还给用户
            $serialno = refuse_serialNO();
            $log = array('uid' => $goods_order['uid'], 'user_name' =>$goods_order['user_name'], 'money' => $goods_order['pay_money'], 'method' => $goods_order['id'], 'type' => 2/* 保存到消费记录中 */, 'addtime' => $_timestamp, 'status' => 1, 'serialno' => $serialno,'account'=>'[退款]'.$goods_order['title'].'被拒绝的');
            lib_database::insert('seller_changelog', array_keys($log), $log);
            //增加用户金额
            lib_database::wquery('update ' . tname('users_seller_session') . ' set `money`=`money`+' . $goods_order['pay_money'] . ' where uid=' . $goods_order['uid']);
            lib_database::wquery('update ' . tname('users_seller_fields') . ' set `money`=`money`+' . $goods_order['pay_money'] . ' where uid=' . $goods_order['uid']);
        }
    }else{
        //获取用户邮箱和宝贝数据 用已发送邮件
        $goods_order = lib_database::get_one('select a.id,a.title,a.status,b.uid,b.user_name,b.email,b.mobile from ' . tname('goods') . ' as  a left join ' . tname('users') . ' as b ON a.uid=b.uid WHERE a.id=' . $data['id']);
        $goods_order['user_name']=empty($goods_order['user_name'])?(empty($goods_order['email'])?$goods_order['mobile']:$goods_order['email']):$goods_order['user_name'];
        $goods_order['url']=  u('business','list');
    }
    // 获取旧数据
    $oldData = lib_database::get_one('SELECT * FROM ' . tname('refuse') . ' WHERE id=' . $data['id']);
    // 旧数据存在 执行修改
    if ($oldData) {
        // 修改状态
        lib_database::update($type, array('status' => -1, 'pay_serialno' => ''), 'id=' . $data['id']);
        // 修改拒绝理由 
        lib_database::update('refuse', array('refuse' => $data['refuse']), 'id=' . $data['id']);
        // 旧数据不存在 执行添加
    } else {
        // 修改状态
        lib_database::update($type, array('status' => -1, 'pay_serialno' => ''), 'id=' . $data['id']);
    ...
    // 发送拒绝邮件
    if(!empty($goods_order['email'])){
        send_refuse_email($goods_order['email'],array('user_name'=>$goods_order['user_name'],'goods_name'=>$goods_order['title'],'refuse_reason'=>$data['refuse'],'url'=>$goods_order['url']));
    }
}
