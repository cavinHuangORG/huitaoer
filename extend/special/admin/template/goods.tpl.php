...
<div class="table">
  	...
  	<div class="th">
             <select name="sid">
               ...
               <option value="-<?=$value['sid'];?>" <?php if(request('sid',0)==-$value['sid']){ ?>selected<?php } ?>><?=$value['title'];?></option>
               <?php } ?>
             </select>
             <select name="channel">
          	   <option value="">频道</option>
          	   <?php foreach ($goodnav as $key=>$value){ ?>
               <option value="-<?=$value['id'];?>" <?php if(request('channel',0)==$value['id']){ ?>selected<?php } ?>><?=$value['name'];?></option>
               <?php } ?>
             </select>
             <select name="cat">
...
<form method="POST" action="<?=$_extend_url;?>&pmod=del" onsubmit="return confirmdel();">
    <div class="table">
        ...
        <tr>
        	...
        	<th width="100" class="text-center">专场/展位</th>
        ...
        <tr id="data_<?=$value['id'];?>">
        	...
        	<td class="text-center">
        		...
        		<br/>
        		<span style="color:red"><?=$special_position[$value['pay_id']]['name'];?></span>
        	</td>
