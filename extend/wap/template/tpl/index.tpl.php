...
<section class="deals" id="stage">
    ...
    <div>
	    ...
	    <aside>
	        ￥<span><?=trim_last0(($value['promotion_price']-$value['coupon_money']));?></span>
	    ...
	    <?php if (!empty($value['mcoupon_url'])): ?>
			<span class="tmh_yhq_icon">
				<a href="<?=$value['mcoupon_url']?>"><span><?=$value['coupon_money']?>元优惠劵</span>
				<br>点击领取</a>
			</span>
        <?php endif ?>
	</div>
		...
		success:function(json){
			if(!json.isover){
				...
				for(var i=0,l=goods.length;i<l;i++){
					...
					if (goods[i].is_quan) {
						quancolor = '<span class="quancolor">券后价</span>';
						quan = '<span class="tmh_yhq_icon"><a href="'+goods[i].mcoupon_url+'"><span>'+goods[i].coupon_money+'元优惠劵</span><br>点击领取</a></span>';
					};
					$("#stage").append("<div><a class='imgd' href='"+goods[i].url_format+"'><img width='140'  src='"+goods[i].pic+"'/>"+newstr+"</a><h2><span><a href='"+goods[i].url_format+"'>"+goods[i].title+"</a></span></h2><aside>￥<span>"+goods[i].promotion_price+"</span>"+quancolor+"</aside><p><del>￥"+goods[i].price+"</del><cite>"+goods[i].discount+"折</cite> "+str+"</p>"+quan+"</div>");
				}
			}else{
				$("#Disb").hide();
			}
