...
function goodslist($wherestr=array(),$order='',$start=0, $num=PAGE,$refuse=false){
	...
	if(!$refuse){
		...
		$order=empty($order)?goodsorder():$order;
	}else{
	...
	//拒绝理由
		...
		$query = lib_database::rquery('select a.*,c.user_name,FROM_UNIXTIME(a.`start`,"%Y%m%d") as day,b.refuse from '.tname('goods').' as a left join '.tname('refuse').' as b ON a.id=b.id LEFT JOIN ' . tname('users') . ' AS c ON a.uid=c.uid where '.$wherestr);
		while ($rt = lib_database::fetch_one())
		{
			$data[] = $rt;
		}
...
function exclist($wherestr=array(),$order='`sort` DESC',$start=0, $num=20,$refuse=false){
	...
	if(!$refuse){
		...
		$data=array();
		$query = lib_database::rquery('select *,a.id as exchange_id  from '.tname('exchange').' as a left join '.tname('refuse').' as b ON a.id=b.id where '.$wherestr);
		while ($rt = lib_database::fetch_one())
		{
			$data[] = $rt;
		}
...
function comment(){
	...
	if($comment['idtype']=='try'){
		...
		}elseif(is_trygoods($try['id'])){//判断是否有申请过该商品的试用
			message('-1','提示',"您已申请过该试用商品，请勿重复申请",u('try','detail',array('id'=>$id)));
		}
...
function goodsorder($sort=''){
	if($sort=='new'){
		return '`sort` desc,`start` desc,`isrec` desc';//按开始时间排序
	}elseif ($sort=='end'){
		return '`sort` desc,`end` asc,`isrec` desc';//按结束时间排序
	}elseif ($sort=='hot'){
		return '`sort` desc,`volume` desc,`isrec` desc';//按销量排序
	}elseif ($sort=='promotion_price'){
		return '`sort` desc,`promotion_price`-`coupon_money` asc,`isrec` desc';//按销售价排序
	}elseif ($sort=='promotion_price_desc'){
		return '`sort` desc,`promotion_price`-`coupon_money` desc,`isrec` desc';//按销售价排序
	}else{
		return '`sort` desc,`start` desc,`isrec` desc';
	}
}
//设置头像
function setavatar(){
...
//发送拒绝邮件
function send_refuse_email($email,$data){
	global $_webset,$_timestamp;
	$data['site_name']=$_webset['site_name'];
	$data['time']=date('Y-m-d H:i:s',$_timestamp);
	$data['email']=$email;
	//判断有没有开启
	$email_tpl=unserialize($_webset['email_tpl_refuse']);
	if($_webset['email_open']==1 && $email_tpl['open']==1){
		//邮件模板处理
		$tpl=get_email_tpl('refuse',$data);
		send_email($email,$data['site_name'].$email_tpl['title'],$tpl);
	}
	return true;
}
...
function getDiscountUrl($url){
	$url=str_replace('&amp;', '&', $url);
	parse_str(parse_url($url,PHP_URL_QUERY),$info);
	if(isset($info['seller_id'])){
		$info['sellerId']=$info['seller_id'];
	}
	if(isset($info['activity_id'])){
		$info['activityId']=$info['activity_id'];
	}
